...
L_calendar.prototype={
	...
	SetDisplayCor:function(day,obj){
		...
		if(this.InputDate!=null){	
			if(day==this.InputDate.getDate()+parseInt(firstday)-1){//如果是输入日期
				obj.style.background="#2cb82c";obj.style.color="#fff";
				return;
			}		
		} 
	...
	CreateHTML:function(){
		...
		for(var i=0;i<this.L_WDay.length;i++){
			htmlstr+="<li id=\"L_calendar-day_"+i+"\" style=\"background:#fff\" onmouseover=\"this.style.background='#2cb82c'\"  onmouseout=\"parent."+this.NewName+".SetDisplayCor("+i+",this)\"></li>\r\n";
		}
		htmlstr+="</ul>\r\n";
		...
		stylestr+="#L_calendar{border:1px solid f0f0f0;width:158px;padding:1px;height:180px;z-index:9998;text-align:center}\r\n";
		...
		stylestr+="#L_calendar-today{cursor:pointer;float:left;width:63px;height:20px;line-height:20px;margin:1px;text-align:center;background:#2cb82c;color:#fff}"
		stylestr+="</style>";
  		...
  		for (i = 0; i < 39; i++)
		{
			...
			{ 
				if(i<firstday){
					...
					da.innerHTML="<b style=\"\">" + this.L_WDay[i] + "</b>";
					//month=(mm==1?12:mm);
					month=mm;
					day=this.L_WDay[i];
					if(document.all){
				...
				da.style.background=(yy == new Date().getFullYear()&&month==new Date().getMonth()+1&&day==new Date().getDate())? "#FFD700":"#fff";
				if(this.InputDate!=null){
					if(yy==this.InputDate.getFullYear() && month== this.InputDate.getMonth() + 1 && day==this.InputDate.getDate()){
						da.style.background="#2cb82c";da.style.color="#fff";
						}
					}
      		}
  		}
